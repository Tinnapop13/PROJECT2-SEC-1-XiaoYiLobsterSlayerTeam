...
<template>
  <div v-if="modalType === 'AddEmployeeFailed'"  class="w-screen h-screen bg-black/[.8] fixed top-0 left-0 flex items-center justify-center">
    <div class="h-[60vh] w-[30vw] bg-white rounded-xl flex flex-col items-center justify-evenly p-4">
      ...
      <div v-if="newEmployee.FakeName.trim().length === 0" class="text-red-500">Please Insert Name</div>
      <div v-if="newEmployee.PositionRank.trim().length === 0" class="text-red-500">Please Insert Rank</div>
      <div v-if="newEmployee.Comment.trim().length === 0" class="text-red-500">Please Insert Comment</div>
      <div v-if="Number(newEmployee.Age) > 60 || Number(newEmployee.Age) < 20" class="text-red-500">Age must be value between 20 - 60</div>
  ...
  <div v-if="modalType === 'AddEmployeeSuccess'"  class="w-screen h-screen bg-black/[.8] fixed top-0 left-0 flex items-center justify-center">
    <div class="h-[60vh] w-[30vw] bg-white rounded-xl flex flex-col items-center justify-evenly p-4">
      <div class="text-5xl"> Result</div>
      <div  class="text-5xl text-green-500">ADD EMPLOYEE SUCCESS</div>
      <button class="btn-primary btn" @click="$emit('closeModal')">Close</button>
    </div>
  </div>
  ...
  <div v-if="modalType === 'SelectingProfile'"
    ...
    <div class="h-[60vh] w-[30vw] bg-white rounded-xl flex flex-col items-center justify-evenly p-4">
      <div class="text-4xl">What do you feel?</div>
      <div class="flex ">
  ...
  <div v-if="modalType === 'DeleteConfirmation'"  class="w-screen h-screen bg-black/[.8] fixed top-0 left-0 flex items-center justify-center">
    <div class="h-[60vh] w-[30vw] bg-white rounded-xl flex flex-col items-center justify-evenly p-4">
      ...
      <div  class="text-5xl text-green-500">Are you sure to Delete Employee {{ deleteEmployeeName }}</div>
